...
describe("KashiPair Basic", function () {
    ...
    beforeEach(async function () {
        ...
        tokenA = await ReturnFalseERC20Mock.deploy("Token A", "A", 8, getBigNumber(1000000, 8));
        tokenB = await RevertingERC20Mock.deploy("Token B", "B", 18, getBigNumber(1000000, 18));
        ...
        console.log('CHEGOU AQUI SERÁ?', pair)
        ...
        //Deploy KashiPair
        let KashiPair = await ethers.getContractFactory("KashiPairMock");
        let kashiPair = await KashiPair.deploy(bentoBox.address)
        ...
        await kashiPair.setSwapper(sushiSwapSwapper.address, true)
        await kashiPair.setFeeTo(alice.address)
    ...
    //         expect("Kashi Medium Risk Token A/Token B-Test").to.be.equal("Kashi Medium Risk Token A/Token B-Test")
    //     })
    //     it("Assigns a symbol", async function () {
    //         expect("kmA/B-TEST").to.be.equal("kmA/B-TEST")
    //     })

    //     it("Assigns decimals", async function () {
    //         const numero = 8;
    //         expect(numero).to.be.equal(8)
    //     })

    //     it("totalSupply is reachable", async function () {
    //         const numero = 0;
    //         expect(numero).to.be.equal(0)
    //     })
    // })
