...
let alice, bob, fred: SignerWithAddress;
...
describe("KashiPair Basic", function () {
    ...
    beforeEach(async function () {
        ...
        //Deploy Oracle
        let Oracle = await ethers.getContractFactory("OracleMock");
        let oracle = await Oracle.deploy()
        await oracle.deployed();

        //Deploy SushiSwapSwapper
        let SushiSwapSwapper = await ethers.getContractFactory("SushiSwapSwapper");
        let sushiSwapSwapper = await SushiSwapSwapper.deploy(bentoBox.address, uniswapV2Factory.address, await uniswapV2Factory.pairCodeHash())
       ...
       //-----------------------END OF SECOND PART------------------------------
        ...
        pairHelper = await KashiPair.deploy(bentoBox, kashiPairContract, KashiPairFactory, tokenA, tokenB, oracle, oracleData)
         ...
         await expect(bentoBox.connect(fred).deposit(tokenA.address, fred.address, fred.address, getBigNumber(100), 0))
    ...
    //         await expect(
    //             cmd.addKashiPair("pairHelper", bentoBox, this.kashiPair, ADDRESS_ZERO, tokenB, this.oracle, oracleData)
    ...
    //         const nonce = await tokenA.nonces(this.alice.address)
    //         const deadline = (await this.alice.provider._internalBlockNumber).respTime + 10000
    //         await pairHelper.tokenPermit(tokenA, this.alice, this.alicePrivateKey, 1, nonce, deadline)
    //     })
    // })
    ...
    //         await bentoBox.deposit(tokenB.address, this.alice.address, this.alice.address, 0, getBigNumber(1, 8))
    //         await bentoBox.transfer(tokenB.address, this.alice.address, pairHelper.contract.address, getBigNumber(1, 8))
    ...
    //         await tokenB.connect(this.carol).approve(bentoBox.address, 300)
    ...
    //         ])
    //         expect(await bentoBox.balanceOf(tokenA.address, this.alice.address)).to.be.equal(getBigNumber(50))
    //     })
    ...
    //             cmd.depositCollateral(getBigNumber(100)),
    //             cmd.borrow(sansBorrowFee(getBigNumber(75, 8))),
    //             cmd.do(bentoBox.deposit, tokenB.address, this.alice.address, this.alice.address, getBigNumber(70, 8), 0),
    ...
    //         await pairHelper.run((cmd) => [cmd.unwind(this.swapper, borrowPart, collateralShare)])
    //     })
    // })
    ...
    //                         [tokenB.address, this.alice.address, getBigNumber(25, 8), 0]
    //                     ),
    //                     defaultAbiCoder.encode(
    //                         ["address", "bytes", "bool", "bool", "uint8"],
    //                         [this.externalFunctionMock.address, data.slice(0, -128), true, true, 1]
    //                     ),
    //                     defaultAbiCoder.encode(["address", "address", "int256", "int256"], [tokenB.address, this.alice.address, -1, 0]),
    //                 ]
    //             )
    //         )
    ...
    //         // (25 / 2) + (37.5 / 2) = 31.25
    //         expect(await bentoBox.balanceOf(tokenB.address, this.alice.address)).to.be.equal("3125000000")
    //     })
    ...
    //                     defaultAbiCoder.encode(
    //                         ["address", "address", "int256", "int256"],
    //                         [tokenB.address, this.alice.address, getBigNumber(25), 0]
    //                     ),
    //                 ]
    //             )
    //         )
    ...
    //             [0],
    //             [defaultAbiCoder.encode(["address", "address[]", "uint256[]"], [tokenB.address, [this.carol.address], [getBigNumber(10, 8)]])]
    //         )
    //     })

    //     it("allows to addAsset with approval", async function () {
    //         const nonce = await bentoBox.nonces(this.alice.address)
    //         await expect(
    //             await pairHelper.run((cmd) => [
    //                 cmd.approveAsset(getBigNumber(100, 8)),
    ...
    //             cmd.borrow(sansBorrowFee(getBigNumber(75, 8))),
    //             cmd.accrue(),
    //             cmd.do(bentoBox.connect(this.bob).deposit, tokenB.address, this.bob.address, this.bob.address, getBigNumber(20, 8), 0),
    //         ])
